...
{
    NSCalendar *gregorian = [NSCalendar currentCalendar];
    NSDateComponents *weekdayComponents = [gregorian components:COMPONENT_FORMAT fromDate:today];
    weekdayComponents.day += 7 - weekdayComponents.weekday;
...
#pragma mark - Duy Pham
- (NSDate *)roundToCurrentHour
{
    NSTimeInterval currentTI = [self timeIntervalSince1970];
    NSTimeInterval roundDownTI = fmod(currentTI, D_HOUR);
    return [self dateByAddingTimeInterval:(-roundDownTI)];
}

- (NSDate *)roundToNextHour
{
    return [[self roundToCurrentHour] dateByAddingTimeInterval:D_HOUR];
}
...
- (NSInteger)differenceInDaysWithDate:(NSDate *)date {
    return [[self dateAtStartOfDay] daysAfterDate:[date dateAtStartOfDay]];
}
...
+ (NSString *)stringForTimeInterval:(NSTimeInterval)ti {
    ...
    NSDateComponents *components = [currentCalendar components:(NSCalendarUnitDay | NSCalendarUnitHour | NSCalendarUnitMinute | NSCalendarUnitSecond) fromDate:[NSDate date] toDate:[NSDate dateWithTimeIntervalSinceNow:ti] options:0];
